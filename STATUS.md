...
**Total commits:** 136
**Total tests:** **353** (273 Rust workspace + 80 `wallet-adapter`); 10 ignored gates that require external resources (testnet RPC, INTEGRATION=1).
...
| Phase 7 | Wallet integration (SEP-43 adapter; Freighter + passkey-kit) | Complete with **human-gated outstanding** | `INTEGRATION=1 pnpm test` green; Phase 7 Round 2 OZ-SA `AuthPayload` encoder shipped. On-chain submission of the encoded payload is gated on the Phase 10 mainnet canary — see [`walkthroughs/phase7-testnet-install/BLOCKER.md`](walkthroughs/phase7-testnet-install/BLOCKER.md). | `f8d2aa1` → `bd60009` |
...
- **Combined:** 353 tests, 10 ignored gates.
...
- [`walkthroughs/phase7-testnet-install/`](walkthroughs/phase7-testnet-install/)
  ...
  account. Includes [`BLOCKER.md`](walkthroughs/phase7-testnet-install/BLOCKER.md)
  detailing the OZ `AuthPayload` encoding work, which Stream B of Phase
  7 Round 2 resolved at the code level (`wallet-adapter/src/oz_smart_account_auth.ts`).
...
1. **MCP `verify_install` returns synthetic drift.** The on-chain
   readback path is documented in
   [`walkthroughs/phase7-testnet-install/BLOCKER.md`](walkthroughs/phase7-testnet-install/BLOCKER.md);
   Stream B's `wallet-adapter/src/oz_smart_account_auth.ts` resolved
   the *write* path (the `AuthPayload` encoder + `auth_digest`
   computation). The *read* path is gated by the Phase 10 mainnet
   canary (see [`docs/mainnet-readiness.md`](docs/mainnet-readiness.md)
   §2 step 8 — the caveat is explicit).
2. **External audit not engaged.** Every prerequisite box in
...
3. **Hosted MCP endpoint not deployed.** The blueprint at
   ...
   bearer-token secret).
4. **`v1.0.0` not tagged.** The release workflow at
...
5. **`SECURITY.md` still has placeholders.** The disclosure email and
   the GPG fingerprint are explicitly tagged `<placeholder>` (lines 18
   and 23). The runbook references them; the human operator replaces
   them when minting the v1.0.0 release key.
6. **Mainnet canary has not been run.** The `walkthroughs/mainnet-canary/`
