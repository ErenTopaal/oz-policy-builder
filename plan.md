...
- `cargo run -p oz-policy-cli -- prepare-install <that spec> --smart-account <testnet smart account> --source <funded source> --rpc <testnet RPC>` returns a base64 XDR envelope; the `prepare-install` output's `envelope_xdr_base64` decodes via `stellar_xdr::curr::TransactionEnvelope::from_xdr_base64` and contains exactly **one** `InvokeHostFunction` op invoking `add_context_rule(context_type, name, valid_until, signers, policies: Map<Address, Val>)` — policies are passed inline per `stellar-accounts` v0.7.1's actual signature, not via separate `add_policy` calls. (Reasoning: `add_policy(context_rule_id, ...)` requires a `context_rule_id` that doesn't exist before installation. See `docs/oz-internal-shapes.md` §6.)
  - Note: The Phase 2 `prepare-install` test path currently returns `Error::InstallPreflightFailed('primitive_address_unknown ...')` until either (a) `stellar-accounts` ships a canonical-address registry, or (b) a project-specific deployment registry is added under `crates/oz-policy-installer/src/registry.rs`. Track as Phase 7 dependency.
...
### Verification / Test / Validation
- The 3 walkthrough corpora (`walkthroughs/01-blend-yield/`, `walkthroughs/02-sep41-subscription/`, `walkthroughs/03-soroswap-bounded/`) are frozen by Stream A: each carries the recording (the recorded transaction JSON), `expected-spec-auto.json` (and `expected-spec-track-a.json` where compose-only is expressible), `wasm/slot_*/{source.rs,policy.wasm,wasm_hash.txt}` for Track-B slots, `expected-sim-report.json`, and a README narrative. Corpora are APPEND-ONLY; any rotation of a recording requires updating every dependent artefact in the same commit.
- The `.github/workflows/walkthroughs.yml` workflow re-derives recording → spec → sim-report byte-equally on every PR. The workflow is a discovery loop over `walkthroughs/[0-9][0-9]-*/` directories that own an `expected-spec-auto.json`; it runs the **pure-in-process** `synthesize` (auto + compose-only when applicable), `codegen` (gated on the presence of `wasm/slot_*/wasm_hash.txt`), and `simulate` (gated on the presence of `expected-sim-report.json` + the canonicaliser in `walkthroughs/canonicalize-sim-report.sh`).
- **Manual envelope-XDR structural check (skipped in CI).** `oz-policy-installer::build_install_envelope` calls `simulateTransaction`, whose returned resource-fee and footprint values depend on the live testnet ledger — so re-derivation cannot be byte-equal across runs. The workflow instead validates that `expected-install-envelope.xdr`, when present, is readable; full structural diff is a developer-local task (decode both envelopes via `stellar-xdr decode` and diff `InvokeHostFunctionOp.hostFunction.args[]`).
- Recording re-derivation (`oz-policy-cli record --hash …`) is **also** skipped in CI: live testnet RPC retention is ≤7 days, so any recording older than that would fail with `RecorderHashNotFound`. Recordings are frozen at corpus-freeze time and the on-chain provenance is checkable manually via `https://stellar.expert/explorer/testnet/tx/<hash>`.
- **Phase 7 AuthPayload BLOCKER (`walkthroughs/phase7-testnet-install/BLOCKER.md`) is closed by Stream B of Phase 8.** The `wallet-adapter` package now exports `buildOzAuthEntry`, `computeAuthDigest`, `encodeAuthPayload`, and the higher-level `makeOzSmartAccountAuthEncoder`, and `installPolicy` carries an `ozAuthPayloadEncoder?` hook that injects an OZ-SA `AuthPayload` ScVal into any `SorobanCredentials::Address(<SA>)` auth entry before `sendTransaction`. The Phase 7 integration test (`wallet-adapter/src/phase7_integration.test.ts`, gated by `INTEGRATION=1` + the SA-owner secret) now wires that encoder; the happy-path branch is reachable once Stream A confirms which walkthroughs hit the BLOCKER on testnet replay (the install-envelope artefacts in each walkthrough's `expected-install-envelope.xdr` are consumed by an OZ SA, so all three walkthroughs would have hit the BLOCKER under the pre-Stream-B `installPolicy`; with the encoder hook wired they no longer do).
- For each walkthrough, the README.md contains a plain-English description of what the synthesized policy *does and does not* permit, matched to the spec's constraints.

**Completion Criterion (binary):** `.github/workflows/walkthroughs.yml` job is green on the merge commit AND for each of the 3 walkthroughs, `cargo run -p oz-policy-cli -- synthesize <recording> --mode auto > /tmp/spec.json && diff /tmp/spec.json walkthroughs/<n>/expected-spec-auto.json` returns no output. (The recording path varies per walkthrough — `expected-recording.json` for walkthrough 01, `recording.json` for walkthrough 02 — the workflow discovers it via a `[ -f ]` fall-through; the `--rule-name` and `--lifetime` flags are recovered from the frozen spec via `jq` so the diff is exact.)
